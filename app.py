...
app = Flask(__name__, secret_key=os.urandom(12))
app.config.update(
...
def home():
    if session.get('logged_in'):
        ...
        jammaLink = "https://jammacomments.herokuapp.com?" + \
        ...
        if request.method == "POST":
            ...
            if esername in userAccounts:
                if userAccounts[esername]["userpass"] == eserpass:
                    ...
                    logging.info(
                        f"LOGIN SUCCESS    username: {esername}   password: {eserpass}    IP Address: {visitor_ip_address}")
                    ...
                    logging.warning(
                        f"INCORRECT PASSWORD    username: {esername}   password: {eserpass}")
                    return render_template("login.html", error=error)

            else:
                error = 'User does not exist'
                logging.warning(
                    f"USER DOES NOT EXIST    username: {esername}   password: {eserpass}")
                return render_template("login.html", error=error)
        ...
        if request.method == "POST":
            ...
            if 'userpicture' in request.files:
                ...
                if eserpicture and allowed_file(eserpicture.filename):
                    ...
                    image.save(os.path.join(
                        "static/userpictures", newfilename))

                    bucket = storage.bucket()
                    blob = bucket.blob(os.path.join(
                        "userpictures/", newfilename))
                    blob.upload_from_filename(os.path.join(
                        "static/userpictures", newfilename), content_type="image/png")
                    blob.make_public()
                    profile_url = (blob.public_url)
                    logging.info(
                        f"{esername} successfully uploaded a profile picture")
            ...
            if esername not in userAccounts:
                if eserpass == eserpass2:
                    ...
                    session['message'] = "You're now registered " + \
                        esername + " please login"
                    visitor_ip_address = request.remote_addr
                    logging.info(
                        f"REGISTRATION SUCCESS    username: {esername}   password: {eserpass}    IP Address: {visitor_ip_address}")
                    return redirect(url_for('index'))
